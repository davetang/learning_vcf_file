...
We can tell `split-vep` to use the `ANN` field using the `-a` parameter but it will tell us that the header is not defined in the expected format. (The ` || true` is added to prevent GitHub Actions from failing.)
...
We can use `bcftools reheader` to modify the header, to make it `split-vep` compatible using some Perl.
