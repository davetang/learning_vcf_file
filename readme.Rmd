...
```{bash engine.opts='-l'}
bcftools filter -i 'TYPE="bnd"' eg/1kgp.bcf | grep -v "^#"
```
...
```{bash engine.opts='-l'}
bcftools filter -i 'TYPE="overlap"' eg/1kgp.bcf | grep -v "^#"
```
...
```{bash engine.opts='-l'}
bcftools filter -i 'GT="1|1"' eg/1kgp.bcf | grep -v "^#" | head -3
```

Genotypes using 2nd alternate allele.

```{bash engine.opts='-l'}
bcftools filter -i 'GT~"2"' eg/1kgp.bcf | grep -v "^#" | head -3
```
